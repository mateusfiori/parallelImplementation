...
__global__ void preencheMatrizDissParalelo(float matrizDiss[][91], float grupoG[], int qteElementos)
{
	int i = blockIdx.x;
	int j = threadIdx.x;

	if (i < qteElementos && j <= qteElementos) {
		matrizDiss[i][j] = sqrt(((grupoG[i * 3] - grupoG[j * 3]) * (grupoG[i * 3] - grupoG[j * 3])) + ((grupoG[i * 3 + 1] - grupoG[j * 3 + 1]) * (grupoG[i * 3 + 1] - grupoG[j * 3 + 1])));
		matrizDiss[i][qteElementos] = grupoG[i * 3 + 2];
	}
}
...
void coletaDadosExternos(int dadosExternos[][2]) {
	...
	arqExterno = fopen("C:\\Users\\cliente\\Desktop\\cudaProjects\\dianaParaleloAVG\\dianaParaleloAVG\\duun.txt", "r"); //arquivo cedido pela Hethini (90 pontos)
...
int main() {
	...
	float start, delta;
	float(*d_matrizDissimilaridade)[91], *d_grupoG, grupoG_array[270];
	...
	do {
		...
		cudaMalloc((float**)&d_matrizDissimilaridade, 91 * 90 * sizeof(float));
		...
		preencheMatrizDissParalelo << <grid, block >> > (d_matrizDissimilaridade, d_grupoG, qteElementos);
		cudaDeviceSynchronize();

		cudaMemcpy(matrizDissimilaridade, d_matrizDissimilaridade, (90 * 91) * sizeof(float), cudaMemcpyDeviceToHost);
		...
		cudaFree(d_grupoG);
		cudaFree(d_matrizDissimilaridade);
